...
- `src/components/profile/edit/ProfileEditView.tsx` (統合編集対応・スマホ最適化版)
...
**1. プロフィール編集の統一・スマホ最適化**

- 基本情報（名前・画像）と詳細情報（アメリカ到着日・家族構成）の編集を1箇所に統一
- スマホ前提のシンプルなUI/UX設計
- 不要な機能（自己紹介、居住エリア、趣味・関心事、話せる言語）を削除
- ユーザーエクスペリエンスの向上

**2. 画像アップロードの最適化**

- 写真撮影機能を削除（スマホでは不要）
- ドラッグ&ドロップ機能を削除（スマホでは使用しない）
- シンプルな画像選択・アップロード機能に最適化

**3. バリデーションの強化**

- アメリカ到着日の未来日付設定を防止
- 極端に過去の日付設定を制限（100年前より前は設定不可）
- リアルタイムバリデーションとエラー表示

**4. 状態管理の最適化**

- 強制再読み込み（`window.location.reload()`）を削除
- `useProfileDetails`フックの`reload`関数を使用した効率的な状態更新
- プロフィール編集後の即座反映
